...
},fsr$setPosition:function(M){FSR.$each(M||{},function(O,N){if(!FSR.$defined(O)){delete M[N]}});M=FSR.$merge({relativeTo:document.body,position:{x:"center",y:"center"},edge:false,offset:{x:0,y:0},returnPos:false,relFixedPosition:false,ignoreMargins:false},M);
function L(N){if(FSR.$type(N)!="string"){return N}N=N.toLowerCase();var O={};if(N.fsr$test("left")){O.x="left"
}else{if(N.fsr$test("right")){O.x="right"}else{O.x="center"}}if(N.fsr$test("upper")||N.fsr$test("top")){O.y="top"
}else{if(N.fsr$test("bottom")){O.y="bottom"}else{O.y="center"}}return O}M.edge=L(M.edge);M.position=L(M.position);
if(!M.edge){if(M.position.x=="center"&&M.position.y=="center"){M.edge={x:"center",y:"center"}}else{M.edge={x:"left",y:"top"}
}}this.fsr$setStyle("position","absolute");var J=$fsr(M.relativeTo)||document.body;var E;if(J==document.body){E=window.fsr$getScroll()
}else{E=J.fsr$getPosition()}var I=E.y;var C=E.x;if(I<0){I=0}if(C<0){C=0}var D=this.fsr$getDimensions({computeSize:false,styles:["padding","border","margin"]});
if(M.ignoreMargins){M.offset.x=M.offset.x-D["margin-left"];M.offset.y=M.offset.y-D["margin-top"]}var H={};
var A=M.offset.y.fsr$toInt();var B=M.offset.x.fsr$toInt();var K=window.fsr$getSize();switch(M.position.x){case"left":H.x=C+B;
break;case"right":H.x=C+B+J.offsetWidth;break;default:H.x=C+(((J==document.body)?K.x:J.offsetWidth)/2)+B;
break}switch(M.position.y){case"top":H.y=I+A;break;case"bottom":H.y=I+A+J.offsetHeight;break;default:H.y=I+(((J==document.body)?K.y:J.offsetHeight)/2)+A;
break}if(M.edge){var G={};switch(M.edge.x){case"left":G.x=0;break;case"right":G.x=-D.x-D.computedRight-D.computedLeft;
break;default:G.x=-(D.x/2);break}switch(M.edge.y){case"top":G.y=0;break;case"bottom":G.y=-D.y-D.computedTop-D.computedBottom;
break;default:G.y=-(D.y/2);break}H.x=H.x+G.x;H.y=H.y+G.y}H={left:((H.x>=0)?H.x:0).fsr$toInt(),top:((H.y>=0)?H.y:0).fsr$toInt()};
if(J.fsr$getStyle("position")=="fixed"||M.relFixedPosition){var F=window.fsr$getScroll();H.top=H.top.fsr$toInt()+F.y;
H.left=H.left.fsr$toInt()+F.x}if(M.returnPos){return H}else{this.fsr$setStyles(H)}return this}});
FSR.Element.Properties.styles={set:function(A){this.fsr$setStyles(A)}};FSR.Element.Properties.opacity={set:function(A,B){if(!B){if(A==0){if(this.style.visibility!="hidden"){this.style.visibility="hidden"
...
F.borderColor[C]=F[E][C]=B[C]="rgb(@, @, @)"});FSR.Modalizer=new FSR.Class({defaultModalStyle:{display:"block",position:"fixed",top:0,left:0,"z-index":30000,"background-color":"#333",opacity:0.8},setModalOptions:function(B){var A=window.fsr$getScrollSize();
this.modalOptions=FSR.$merge({width:(A.x+300+"px"),height:(A.y+300+"px"),elementsToHide:"select",onModalHide:FSR.$empty,onModalShow:FSR.$empty,hideOnClick:true,modalStyle:{},updateOnResize:true},this.modalOptions,B);
return this},toElement:function(){return $fsr("fsr_modalOverlay")},resize:function(){if($fsr("fsr_modalOverlay")){var A=window.fsr$getScrollSize();
$fsr("fsr_modalOverlay").fsr$setStyles({width:(A.x+300+"px"),height:(A.y+300+"px")})}},setModalStyle:function(A){this.modalOptions.modalStyle=A;
this.modalStyle=FSR.$merge(this.defaultModalStyle,{width:this.modalOptions.width,height:this.modalOptions.height},A);
...
}else{C.fsr$set("text",A)}}catch(D){dbug.log("error: %s",D)}}.fsr$bind(this))}});FSR.StickyWin=new FSR.Class({Implements:[FSR.Options,FSR.Events,FSR.StyleWriter],options:{closeClassName:"closeSticky",pinClassName:"pinSticky",content:"",zIndex:40000,className:"",width:false,height:false,timeout:-1,allowMultipleByClass:false,allowMultiple:true,showNow:false,wrapWithUi:false,caption:"",uiOptions:{},useIframeShim:true,iframeShimSelector:""},css:'.SWclearfix:after {content: "."; display: block; height: 0; clear: both; visibility: hidden;}.SWclearfix {display: inline-table;}* html .SWclearfix {height: 1%;}.SWclearfix {display: block;}',initialize:function(A){this.setOptions(A);
...
if(!$fsr(this.id)){this.win=new FSR.Element("div",{id:this.id}).fsr$addClass(this.options.className).fsr$addClass("StickyWinInstance").fsr$addClass("SWclearfix").fsr$setStyles({display:"none",position:"absolute",zIndex:this.options.zIndex}).fsr$inject(document.body,"top").fsr$setProperty("alt","Survey Invitation").fsr$store("StickyWin",this)
...
},update:function(C){this.Request.send({url:C||this.options.url});return this}}}})();FSR.StickyWin.ui=function(C,A,D){if($fsr("fsrwin")){return $fsr("fsrwin")
}D=FSR.$extend({width:300,cornerHandle:false,cssClass:"",baseHref:"",buttons:[],css:""},D);C=FSR.$pick(C,"%caption%");
A=FSR.$pick(A,"%body%");A=A.replace(/{%baseHref%}/g,D.baseHref);var B=new FSR.Element("div",{id:"fsrwin"}).fsr$setStyle("width",D.width).fsr$addClass("fsrwin");
...
if($fsr(A)){E.fsr$adopt(A)}else{E.fsr$set("html",A)}var H=new FSR.Element("div").fsr$addClass("fsr_top_ur").fsr$adopt(new FSR.Element("div",{id:"close"}).fsr$addClass("fsr_closeButton").fsr$addClass("fsr_closeSticky").fsr$addEvent("click",D.close)).fsr$adopt(G);
...
}this.shim.fsr$setStyles({position:"absolute",zIndex:B,border:"none",filter:"progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)"}).fsr$setProperties({src:this.src,frameborder:"0",scrolling:"no",id:this.id,alt:"Survey Invitation Helper Window - Please Ignore"}).fsr$addClass(this.options.className);
