...
// Modified: 4/23/2010  3:00 PM - by OTSO
...
//  Added WT.co_SSA to hold value of WT.co_f for use in WT reports - here for new cookie
		WT.co_SSA=WT.co_f;
...
//  Added WT.co_SSA to hold value of WT.co_f for use in WT reports - here for existing cookie
	WT.co_SSA=WT.co_f;
...
WebTrends.prototype.dcsMultiTrack=function(){
	...
	if (args.length%2==0){
		...
		var date = new Date();
        	var curDate = null;
		do { curDate = new Date(); }
		while(curDate-date < 500); // 500 ms delay
	}
}
...
//Override DCS.dcssip with www.socialsecurity.gov	
		if (DCS.dcssip!="mwww.ba.ssa.gov") {
		DCS.dcssip="www.socialsecurity.gov";
		}
	...
	// changed value to 4 to enable 1st party session cookie 04/30/2010
	// 1 is equal to no cookie, 4 is equal to session cookie
	//DCS.dcscfg=1;
	DCS.dcscfg=4;
}
...
//modified for SSA, make all calls using https
		//	var P="http"+(window.location.protocol.indexOf('https:')==0?'s':'')+"://"+this.domain+(this.dcsid==""?'':'/'+this.dcsid)+"/dcs.gif?";
//modified by SSA, make all calls using http
