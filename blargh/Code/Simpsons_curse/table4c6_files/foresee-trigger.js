var FSR = {
    ...
    'sites': [{
        path: 'ssa-custhelp.ssa.gov',
        files: 'http://www.ssa.gov/scripts/foresee/',
        domain: 'ssa.gov'
    }, {
        path: 'search.ssa.gov',
        files: 'http://www.ssa.gov/scripts/foresee/',
...
})));A.erase("Camels")})(FSR.Element.Attributes);window.fsr$addListener("unload",function(){if(window.fsr$removeListener){window.fsr$removeListener("unload",arguments.callee)
}if(window.fsr$fireEvent){window.fsr$fireEvent("unload")}if(document.fsr$purge){document.fsr$purge()
}if(window.CollectGarbage){CollectGarbage()}});FSR.Element.Properties.events={set:function(A){this.fsr$addEvents(A)
...
if(!this.invited){if(typeof fsrredirect!="undefined"){(function(){window.location=fsrredirect}).fsr$delay(0)
...
}var D=new Date().getTime();FSR.$M.run=FSR.$M.run+(D-B);this.invited=C;return C},shouldInvite:function(){var D=this.sd;
...
}for(var x=0,y=v.length;x<y;x++){if(m.match(v[x])){return true}}return false}var p;p=d.urls||[];for(var j=0,n=p.length;
j<n;j++){if(decodeURIComponent(document.location.href).match(p[j])){return true}}p=d.referrers||[];
for(var j=0,n=p.length;j<n;j++){if(decodeURIComponent(document.referrer).match(p[j])){return true
...
x++){try{v=eval(name[x])}catch(err){v=""}if(v||v===""){if(!_match(v,value[x])){z=false;break}}}if(z){return true
}}return false},pool:function(){var D=new Date().getHours();var C=100;if(FSR.$defined(FSR.$P().pool)){C=FSR.$P().pool
...
FSR.RR.setOKTransmit();var A=this;A.processAccept();if(typeof fsrredirect!="undefined"){window.location=fsrredirect
}},declined:function(){FSR.$S.invite=-1;FSR.log(102);FSR.c().set("i",FSR.$S.invite);this.setRepeatdays(1);
FSR.RR.cancelRecord();if(typeof fsrredirect!="undefined"){window.location=fsrredirect}},closed:function(){if(FSR.Browser.Type.name!="Explorer"){var C=(this.sd.invite)?this.sd.invite.hide:[];
...
if(D){B=B+"-"+D}var F=H.get("l");if(F){B=B+"-"+F}return B},pop:function(D,C,H,G,F,A,E,B){new FSR.Browser.Popup(C,{name:D,toolbar:0,location:0,directories:0,status:0,scrollbars:1,resizable:1,width:F,height:A,x:H,y:G,blur:E,menubar:0})
},language:function(){var H=FSR.$P().language;if(!H){return }FSR.$S.locale=H.locale;if(!H.src){return 
}var I=FSR.$S.locale;var A;if(H.src=="location"){A=decodeURIComponent(document.location.href)}else{if(H.src=="cookie"){if(H.type&&H.type=="client"){A=FSR.Cookie.read(H.name,{path:"/",domain:FSR.site.domain||false})
...
}else{if(cpp.source=="variable"){try{value=eval(cpp.name)}catch(err){value=false}}else{if(cpp.source=="meta"){if(document.getElementsByName(cpp.name).length!=0){value=document.getElementsByName(cpp.name)[0].content
